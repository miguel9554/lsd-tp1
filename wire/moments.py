...
def rc_line_moments(R: int, C: int, sections: int, max_moment: int):
    state_vector_length = sections + 2
    ...
    for order in range (1, max_moment+1):
        ...
        moments[state_vector_length-1, order] = -np.sum(moments[1:state_vector_length-1, order-1])*C
