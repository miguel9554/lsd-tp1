...
#
# --> tau_por_CL: cantidad de veces que se vario la constante de tiempo de entrada
#                 para una misma capacidad de carga equivalente en la tabla que modela
#                 al inversor o al FF
...
class Estimador:
	...
	def __init__(self, modelo_pi, Tau_in, Vdd, nombre_tabla_timing, umbral_err, tau_por_CL):
	...
	def estimar_retardo_rise(self):
		...
		while error_relativo > self.umbral_err:
			...
			[t_50_sig, t_20, t_LH] = self.buscar_en_tabla(self.tabla_timing, CL, self.Tau_in, self.tau_por_CL, False);
	...
	def estimar_retardo_fall(self):
		...
		while error_relativo > self.umbral_err:
			...
			# Buscar en la tabla del FF o del inversor
			# [t_50, t_20] = buscar_en_tabla_FF(CL);
			[t_50_sig, t_20, t_HL] = self.buscar_en_tabla(self.tabla_timing, CL, self.Tau_in, self.tau_por_CL, True);
	...
	def buscar_en_tabla(self, tabla_timing, CL, Tau_in, tau_por_CL, es_rise_time):
			err_rel_CL = 1
			err_rel_tau = 1
